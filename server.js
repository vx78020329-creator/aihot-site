...
app.get('/feed.xml', (req, res) => {
  res.type('application/rss+xml').send(buildRSS('全球热点-精选','Curated', db.prepare('SELECT * FROM items WHERE is_curated=1 ORDER BY score DESC LIMIT 50').all()));
});
app.get('/feed/all.xml', (req, res) => {
  res.type('application/rss+xml').send(buildRSS('全球热点-全部','All', db.prepare('SELECT * FROM items ORDER BY published_at DESC LIMIT 100').all()));
});
app.get('/feed/daily.xml', (req, res) => {
  res.type('application/rss+xml').send(buildRSS('全球热点-日报','Daily', db.prepare("SELECT * FROM items WHERE collected_at>=date('now') ORDER BY score DESC LIMIT 50").all()));
});
